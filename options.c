...
static struct option abook_vars[] = {
  ...
  { "backup", OT_BOOL, BOOL_BACKUP, TRUE },
  { "address_book", OT_STR, STR_ADDRESS_BOOK, UL "" },
...
check_options()
{
  ...
  str = opt_get_str(STR_ADDRESS_BOOK);
  if (*str) {
    if(stat(str, &s) == -1) {
      fprintf(stderr, _("%s does not exists\n"), str);
      restore_default(&abook_vars[STR_ADDRESS_BOOK]);
      err++;
    }
  }
