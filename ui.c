...
opt_color_to_color(enum str_opts enum_name)
{
  ...
  else CHECK_COLOR_NAME(name, "brblack", 8)
	else CHECK_COLOR_NAME(name, "brred", 9)
	else CHECK_COLOR_NAME(name, "brgreen", 10)
	else CHECK_COLOR_NAME(name, "bryellow", 11)
	else CHECK_COLOR_NAME(name, "brblue", 12)
	else CHECK_COLOR_NAME(name, "brmagenta", 13)
	else CHECK_COLOR_NAME(name, "brcyan", 14)
	else CHECK_COLOR_NAME(name, "brwhite", 15)
  else if (strncmp(name, "color", 5) == 0) {
    // $name begins with "color", now check the number
    uint idx = atoi(name + 5);
    if (idx < COLORS)
      return idx;
  }
